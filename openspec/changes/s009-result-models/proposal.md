...
應用程式的照片下載流程分為兩個階段：抓取照片資訊與批次下載照片檔案。每個階段需要對應的結果模型來封裝操作結果。`FetchResult` 封裝照片資訊抓取結果（照片列表、部落格 ID、標題、是否完全快取），`DownloadBatchResult` 封裝批次下載結果（成功數、失敗照片列表、是否全部成功）。這兩個模型讓 ViewModel 層能清楚掌握操作結果並更新 UI 狀態。
